...
/* Set up a registry of player infor and their socket id */
let players = [];
...
io.on('connection', (socket) => {
    /* Output a lof message on the server and send it to the clients */
    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        io.in(room).fetchSockets().then((sockets) => {
            /* serverLog('There are ' + sockets.length + ' clients in the room, ' + room);*/   /*for testing, not needed anymore*/
    ...
    socket.on('uninvite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not request a valid user to invite to uninvite'
            }
            socket.emit('uninvited', response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        /* set up when last legal game was made */
        let d = new Date();
...
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to see if opposite color  is present */
